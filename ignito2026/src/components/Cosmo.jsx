...
const RULES = [
  {
    ...
    reply: 'You can register through the "Register" button in the navigation, or scroll to the Mission Control section at the bottom of the page.',
  },
  {
    match: /(when|date|time|schedule)/i,
    reply: 'IGNITO 2026 launches in March 2026. Check the Countdown and Timeline sections for the exact schedule.',
  },
  {
          ...
          >
            <div className="flex items-center gap-2 border-b border-white/10 px-4 py-3">
              <div className="flex h-8 w-8 items-center justify-center rounded-full bg-gradient-to-br from-cyan to-plasma">
                <Bot className="h-4 w-4 text-void" />
              </div>
                ...
                <div key={i} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                  <div
                    className={`max-w-[85%] rounded-2xl px-3.5 py-2 text-sm ${
                      m.role === 'user'
                        ? 'bg-gradient-to-r from-ion to-plasma text-white'
