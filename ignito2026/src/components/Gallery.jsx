      ...
      <div className="mx-auto grid max-w-6xl grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4">
        {GALLERY.map((g, i) => (
          <motion.button
            ...
            className={`group relative aspect-square overflow-hidden rounded-xl bg-gradient-to-br ${g.hue} text-left`}
            aria-label={`Open image: ${g.caption}`}
          >
            <div className="absolute inset-0 flex items-center justify-center bg-black/20 opacity-0 transition group-hover:opacity-100">
              <ImageIcon className="h-6 w-6 text-white" />
            </div>
            <span className="absolute bottom-0 left-0 right-0 translate-y-full bg-black/60 p-2 text-xs text-white transition group-hover:translate-y-0">
              {g.caption}
            </span>
          </motion.button>
        ))}
        ...
        {activeIndex !== null && (
          ...
          >
            ...
            <button
              ...
              className="absolute left-4 rounded-full p-2 text-white/70 transition hover:bg-white/10 hover:text-white sm:left-8"
            >
            ...
            >
              ...
              <p className="font-display text-white">{GALLERY[activeIndex].caption}</p>
            </motion.div>
            <button
              onClick={next}
              aria-label="Next image"
              className="absolute right-4 rounded-full p-2 text-white/70 transition hover:bg-white/10 hover:text-white sm:right-8"
            >
