            ...
            return (
              <TiltCard key={c.id} className="rounded-2xl" max={6}>
                ...
                >
                  ...
                  <button
                    type="button"
                    onClick={() => openRegistration(c.name)}
                    className="btn-glow group/btn mt-1 flex w-full items-center justify-center gap-1.5 rounded-full bg-ink px-4 py-2.5 text-xs font-bold uppercase tracking-widest text-paper transition hover:scale-[1.02]"
                  >
                    <Rocket className="h-3.5 w-3.5 transition-transform group-hover/btn:-translate-y-0.5 group-hover/btn:rotate-12" />
                    Deploy to mission
                  </button>
                </motion.article>
              </TiltCard>
